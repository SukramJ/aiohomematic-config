...
aiohomematic-config. It is modeled on the
...
gates: the test suite (`pytest tests/`) and all pre-commit hooks (`prek run --all-files`, which
runs ruff, ruff-format, mypy, pylint, bandit, codespell, yamllint, prettier, and the project's
own lint scripts). AI assistance does not lower the bar.

## How this project itself uses AI

In the interest of transparency: aiohomematic-config is partly developed with AI assistance
